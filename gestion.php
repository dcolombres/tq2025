...
function getItem($type, $id) { require __DIR__ . '/db_config.php'; $conn = new mysqli($servername, $username, $password, $dbname); if ($conn->connect_error) return null; $conn->set_charset("utf8mb4"); $table_name = ($type === 'subcategoria') ? 'subcategorias' : $type . 's'; $stmt = $conn->prepare("SELECT nombre FROM $table_name WHERE id = ?"); $stmt->bind_param("i", $id); $stmt->execute(); $stmt->store_result(); $stmt->bind_result($nombre); $item = null; if($stmt->fetch()) { $item = ['id' => $id, 'type' => $type, 'nombre' => $nombre]; } $stmt->close(); $conn->close(); return $item; }
function getCategorias() { require __DIR__ . '/db_config.php'; $conn = new mysqli($servername, $username, $password, $dbname); if ($conn->connect_error) return []; $conn->set_charset("utf8mb4"); $sql = "SELECT id, nombre FROM categorias ORDER BY nombre ASC"; $stmt = $conn->prepare($sql); $stmt->execute(); $stmt->store_result(); $stmt->bind_result($id, $nombre); $items = []; while ($stmt->fetch()) { $items[] = ['id' => $id, 'nombre' => $nombre]; } $stmt->close(); $conn->close(); return $items; }
function getNiveles() { require __DIR__ . '/db_config.php'; $conn = new mysqli($servername, $username, $password, $dbname); if ($conn->connect_error) return []; $conn->set_charset("utf8mb4"); $sql = "SELECT id, nombre FROM niveles ORDER BY nombre ASC"; $stmt = $conn->prepare($sql); $stmt->execute(); $stmt->store_result(); $stmt->bind_result($id, $nombre); $items = []; while ($stmt->fetch()) { $items[] = ['id' => $id, 'nombre' => $nombre]; } $stmt->close(); $conn->close(); return $items; }
function getSubcategoriasStats() { require __DIR__ . '/db_config.php'; $conn = new mysqli($servername, $username, $password, $dbname); if ($conn->connect_error) return [['error' => $conn->connect_error]]; $conn->set_charset("utf8mb4"); $sql = "SELECT s.id AS subcategoria_id, c.nombre AS categoria, n.nombre AS nivel, s.nombre AS subcategoria, s.permite_validacion_externa, (SELECT COUNT(p.id) FROM palabras p WHERE p.subcategoria_id = s.id) AS cantidad_palabras FROM subcategorias s JOIN categorias c ON s.categoria_id = c.id LEFT JOIN niveles n ON s.nivel_id = n.id ORDER BY c.nombre, n.nombre, s.nombre;"; $stmt = $conn->prepare($sql); if (!$stmt || !$stmt->execute()) { return [['error' => $conn->error]]; } $stmt->store_result(); $stmt->bind_result($subcategoria_id, $categoria, $nivel, $subcategoria, $permite_validacion_externa, $cantidad_palabras); $stats = []; while ($stmt->fetch()) { $stats[] = ['subcategoria_id' => $subcategoria_id, 'categoria' => $categoria, 'nivel' => $nivel, 'subcategoria' => $subcategoria, 'permite_validacion_externa' => $permite_validacion_externa, 'cantidad_palabras' => $cantidad_palabras]; } $stmt->close(); $conn->close(); return $stats; }
function getWordsForSubcategory($subcategoria_id) { require __DIR__ . '/db_config.php'; $conn = new mysqli($servername, $username, $password, $dbname); if ($conn->connect_error) return []; $conn->set_charset("utf8mb4"); $sql = "SELECT id, palabra FROM palabras WHERE subcategoria_id = ? ORDER BY palabra ASC"; $stmt = $conn->prepare($sql); $stmt->bind_param("i", $subcategoria_id); $stmt->execute(); $stmt->store_result(); $stmt->bind_result($id, $palabra); $words = []; while ($stmt->fetch()) { $words[] = ['id' => $id, 'palabra' => $palabra]; } $stmt->close(); $conn->close(); return $words; }
